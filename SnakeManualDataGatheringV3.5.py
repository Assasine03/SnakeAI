...
def getInput(LearningData,ind):
	...
	for i in range(len(tab)):
		posX = tab[i][0]
		posY = tab[i][1]
		if posX == baseLeftminX and posY == baseY:
			baseLeft = -1
		if posX == baseRightposX and posY == baseY:
			baseRight = -1
		if posY == baseTopposY and posX == baseX:
			baseTop = -1
		if posY == baseBottomminY and posX == baseX:
			baseBottom = -1
	
	l = [baseLeft, baseTop, baseRight, baseBottom]
	#offset = [0, 1, 2, 3]
	offset = {
		"up" : 0,
		"down": 2,
	...
	direction = lData["HeadDirection"][0]
	if offset[direction] != 0:
		for i in range(offset[direction]):
			karl = l.pop()
			l.insert(0, karl)
	...
	#UnpackedData.append(baseLeft2)
	#UnpackedData.append(baseRight2)
	#UnpackedData.append(baseTop2)
	#UnpackedData.append(baseBottom2)
	#UnpackedData[len(UnpackedData)-1] = None

	Ux = lData["Apple"][0]-baseX
	arcX = 0
	if Ux != 0:
		Uy = baseY-lData["Apple"][1]
		U_ = math.sqrt(Ux**2+Uy**2)
		arcX = math.acos(Ux/U_)/math.pi
		print(arcX)
...
while running == True:
	...
	if running == False:
		...
		getInput(LearningData,len(LearningData["Data"])-1)
		# List
		# Dict: Body-List-Body, Head-List-Head, Apple-List-Apples, HeadDirection-List-WASD, Border-List-Border
